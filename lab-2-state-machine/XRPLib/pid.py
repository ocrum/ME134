...
class PID(Controller):
    ...
    def update(self, error: float, debug: bool = False) -> float:
        ...
        if debug:
            ...
            print(f"Kp contribution: {self.kp * error:.4f}")
            #print(f"Timestep: {timestep:.4f}")
            print(f"Ki contribution: {self.ki * integral:.4f}")
            print(f"Kd contribution: {self.kd * derivative:.4f}")
            print(f"Effort total: {output:.4f}")
